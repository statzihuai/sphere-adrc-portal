...
| 5 money-in idempotent crediting | **blocked by a platform bug.** Connect onboarding completed (`chargesEnabled: true`), "$10 credit pack" product created — but the documented end-user endpoints (`POST /billing/purchase`, `GET /billing/orders`, `/subscribe`, `/subscription`) return the same generic 401 for a fresh end-user JWT that works on `/auth/me` and JWT-authed functions, with or without verified email, and identically with **no** Authorization header — i.e. the billing routes never evaluate the bearer token. Our claim machinery is deployed and fail-closed; nothing to fix on our side. |
